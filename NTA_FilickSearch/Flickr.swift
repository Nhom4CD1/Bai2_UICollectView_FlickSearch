...
class Flickr{
    let processingQueue = OperationQueue()
//Hàm tìm kiếm dựa vào api của Flickr
    func searchFlickrForTerm(_ searchTerm: String, completion : @escaping (_ results: FlickrSearchResult?, _ error : NSError?) -> Void){
       ...
       //1.
        URLSession.shared.dataTask(with: searchRequest, completionHandler: { (data, response, error) in
            ...
            do {
                ...
                OperationQueue.main.addOperation({
                    completion(FlickrSearchResult(searchTerm: searchTerm, searchResults: flickrPhotos), nil)
                })
    ...
    fileprivate func flickrSearchURLForSearchTerm(_ searchTerm:String) -> URL? {
        ...
        //Link nhan API key tu trang chu Flickr
        /*
